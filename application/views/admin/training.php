...
<div class="content">
        <h2 class="content-heading">Data Training</h2>
        ...
        <div class="block">
            ...
            <button type="button" class="btn btn-success mr-5 mb-5 left" id="tomboltambah">
                ...
                <table class="table table-responsive table-hover display nowrap" style="width:100%" id="datatraining">
                    <thead>
    ...
    <div class="modal fade" id="modal-popin" tabindex="-1" role="dialog" aria-labelledby="modal-popin" aria-hidden="true">
        <div class="modal-dialog modal-dialog-popin" role="document">
            ...
            <div class="block-content">
                <?= form_open_multipart('Training/uploaddata') ?>
...
function tampildatatraining() {
    table = $('#datatraining').DataTable({
        ...
        "ajax": {
            "url": "<?= site_url('Training/ambildata') ?>",
            "type": "POST"
        },
...
$(document).ready(function () {
    ...
    $('#tomboltambah').click(function(e){
        $.ajax({
            url: "<?= site_url('Training/formtambah')?>",
            dataType: "json",
            success: function (response) {
                if (response.sukses) {
                    $('.viewmodal').html(response.sukses).show();
                    $('#modaltambahtraining').on('shown.bs.modal', function(e) {
                        $('#nis').focus();
                    })
...
function edit(nis) {
    $.ajax({
        type: 'post',
        url: "<?= site_url('Training/formedit') ?>",
        data: {
            nis: nis
        },
        dataType: "json",
        success: function(response) {
            if (response.sukses) {
                $('.viewmodal').html(response.sukses).show();
                $('#modaledittraining').on('shown.bs.modal', function(e) {
                    $('#nama_siswa').focus();
                })
